...
//	       fractionErr.push_back(tempgr[0]->GetErrorYhigh(pcount[0]));
		  ...
		  pcount[1]++;
	       ...
	       dtemp[1] = TMath::Power(ypval[1] - ypval[0],2)/ypval[0];
	       if( (dtemp[1] > 2.) && (ypval[0] < 1.e-3) && (ypval[1] > 1.e-3) )
		  cout << "Chisquare: Skipping point " << i << " with ypval0 = " << ypval[0] << ", ypval1 = " << ypval[1] << ", resid = " << dtemp[0] << ", chisquare part = " << dtemp[1] << endl;
	       else
	       {
	          chisquare += dtemp[1];
	       }
...
/*	       // Both systematics are present
		  ...
		  else
		  {
	       ...
	       cout << "- Chisquare part = " << dtemp[1] << endl;
