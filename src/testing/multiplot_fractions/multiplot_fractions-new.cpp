...
//   vector<double> systFitPos;
   ...
   if(argc > 2)
   {
      // Two input files to determine residuals between them
      cout << "Input files:" << endl;
      cout << "- " << argv[1] << " (main function for residual)" << endl;
      cout << "- " << argv[2] << " (comparison function)" << endl;
      ...
      stemp[0] = plotdir + "/resid/residuals_" + stemp[4] + "_results.txt";
...
//	       fractionErr.push_back(tempgr[0]->GetErrorYhigh(pcount[0]));
	       ...
	       cout << "- Base = " << fraction[3*pcount[0]] /*<< " + " << fractionErr[2*pcount[0]] << " - " << fractionErr[2*pcount[0]+1]*/ << endl;
		  ...
		  pcount[1]++;
	       ...
	       else
	       {
	          ypval[1] = -1;
	          fraction.push_back(ypval[1]);
                  cout << "No comparison plot for point " << i << endl;
	       }

	       // Calculate the residuals R = (base - comp)/sqrt(base)
               dtemp[0] = (ypval[0] - ypval[1])/TMath::Sqrt(ypval[0]);
...
/*	       // Both systematics are present
		  ...
		  else
		  {
	       ...
	       cout << "- Comp = " << fraction[3*i+1] << endl;
	       cout << "- Diff = " << fraction[3*i] - fraction[3*i+1] << endl;
	       cout << "- Residuals = " << fraction[3*i+2] << endl;
...
//	    grindiffComb[j]->SetPointError(i, 0., 0., 0., 0.);
	    grindiff[j]->SetPoint(i, energy[i]-0.03375+0.0225*j, fraction[3*i+2]);
...
//	 grindiffComb[j]->SetFillColorAlpha(grindiffComb[j]->GetMarkerColor(), 0.6);
...
//	 cout << "- " << systFitNeg[2*j] << " ± " << systFitNeg[2*j+1] << ", " << systFitPos[2*j] << " ± " << systFitPos[2*j+1] << endl;
      ...
      stemp[2] = plotdir + "/resid/residuals_" + stemp[4] + "_TreeS6_combine.pdf";
      ...
      stemp[2] = plotdir + "/resid/residuals_" + stemp[4] + "_TreeS6.pdf";
      c2->SaveAs(stemp[2].c_str());
   ...
   {
      cerr << "Error! No input files supplied. Rerun program and add two input files as arguments (lna_composition_results*.root)." << endl;
      return 1;
   }
