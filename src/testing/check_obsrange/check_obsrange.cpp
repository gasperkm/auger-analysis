...
int InitObservables(vector<string> *observables)
{
...
int main(int argc, char **argv)
{
   ...
   double *limits;
   vector<string> *obsnames = new vector<string>;

   cerr << "Which tree to read (TreeS)? ";
   cin >> stemp[1];

   if(argc > 1)
   {
      nrobs = InitObservables(obsnames);

      limits = new double[2*nrobs];
	 ...
	 nrtrees = GetRootKeys(ifile, "TreeS");

	 for(int i = 0; i < nrobs; i++)
	 {
            cout << "  Reading observable " << obsnames->at(i) << ": ";

/*	    for(int j = 0; j < nrtrees; j++)
	    {*/
/*	       stemp[0] = "TreeS" + ToString(j+1);
               cout << "    Reading tree " << stemp[0] << endl;*/
	       tempTree = (TTree*)ifile->Get(stemp[1].c_str());

	       stemp[0] = obsnames->at(i);
	       dtemp[0] = tempTree->GetMinimum(stemp[0].c_str());
	       dtemp[1] = tempTree->GetMaximum(stemp[0].c_str());
	       cout << "min = " << dtemp[0] << "\tmax = " << dtemp[1] << endl;

               if(dtemp[0] < limits[2*i])
                  limits[2*i] = dtemp[0];
               if(dtemp[1] > limits[2*i+1])
                  limits[2*i+1] = dtemp[1];
//	    }
	 }
...
//	 delete ifile;
      ...
      for(int i = 0; i < nrobs; i++)
      {
         cout << obsnames->at(i) << ": Total min = " << limits[2*i] << "\tmax = " << limits[2*i+1] << endl;
      }
